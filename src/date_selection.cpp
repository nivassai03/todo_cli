...
Element Calendar::Render() {
  ...
  auto year_month_header = text(year_month_str) | center | bold;
  auto weekdays_header =
      hbox({text("Sun") | m_date_size, text("Mon") | m_date_size,
  ...
  return window(text("Calendar") | vcenter | bold | size(HEIGHT, EQUAL, 20),
                {vbox({
                    year_month_header, separatorLight(),
...
Component Calendar::generate_dates_component() {
  ...
  for (const Month &m : yr.m_months_in_year) {
    if (m.m_month == m_month) {
      ...
      DaysRow.push_back(Renderer(separatorLight));
